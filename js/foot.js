...
$('#pc_menu_link').click(()=>{
    (function() {
        $(".layer-shade").removeClass('hide');
		$(".layui-layer").removeClass('hide');
		$(".layui-layer a").click(()=>{
			$(".layer-shade").addClass('hide');
...
function openmenulink(){
	if($('#close_menu').hasClass('opening')){
		document.getElementById("close_menu").click();
		$('#close_menu').removeClass('opening')
	}
	if($(".menulist").hasClass('opening')){
		closemenulist()
	}
    $(".menulink .menucontent").css("bottom","-15rem");
	$(".menulink").addClass('opening')
    $(".menulink").show();
    $(".menulink .menucontent").animate({bottom:"0"});
}
function closemenulink(){
    $(".menulink .menucontent").animate({bottom:"-15rem"});
    $(".menulink").delay(400).hide(1);   
	$(".menulink").removeClass('opening')
}
/*!* menu_free-list */
function openmenu(){
	if($(".menulist").hasClass('opening')){
...
function openmenulist(){
    ...
    $(".menulist .menucontent").css("bottom","-15rem");
	$(".menulist").addClass('opening')
    $(".menulist").show();
    $(".menulist .menucontent").animate({bottom:"0"});
}
function closemenulist(){
    $(".menulist .menucontent").animate({bottom:"-15rem"});
    $(".menulist").delay(400).hide(1);   
	$(".menulist").removeClass('opening')
}
/*!* menu_vod-list */
$('#menu_vod,#topmenubtn').click(()=>{
	if($(".menulist").hasClass('opening')){
		closemenulist()
	}
	if($(".menulink").hasClass('opening')){
	...
	if($('#close_menu').hasClass('opening')){
		...
		document.getElementById("openvodlist").click();
		$('#close_menu').addClass('opening')
	}
})
$('#close_menu').click(()=>{
	$('#close_menu').removeClass('opening')
...
/*!* newhtml-ajax */
	...
	$('#newbtn').addClass('active');
	    setTimeout(function() {$('#newbtn').removeClass('active')}, 1000);
		ajaxRead();
	});
	var page=2;
	var mid=1;   
            ...
            success : function(data){
				if(data.pagecount >= page && page < 10){
					...
					for (var i = 0; i < vod.length; i++) {
						html +='<li class="vodlist_item loading"><a class="vodlist_thumb lazyload" href="/vodplay/'+vod[i].vod_id+'/index_1_1.html" data-original="'+vod[i].vod_pic+'"><span class="play hidden_xs"></span><span class="vodlist_top"><em class="voddate voddate_year">'+vod[i].type.type_name+'</em></span></a><div class="vodlist_titbox"><p class="vodlist_title"><a href="/vodplay/'+vod[i].vod_id+'/index_1_1.html">'+vod[i].vod_name+'</a></p><p class="vodlist_sub">'+vod[i].vod_time+'</p></div></li>';
					}
					var vod_list=$('#newbox');
...
/*!* font-style */
$("#zh_click_s,#zh_click_t").click(function(){ 
    $('#zh_click_s,#zh_click_t').toggle();
});
$("#zh_click_sa,#zh_click_ta").click(function(){ 
    $('#zh_click_sa,#zh_click_ta').toggle();
});
var zh_default = 'n';
var zh_choose = 't';
var zh_expires = 7;
